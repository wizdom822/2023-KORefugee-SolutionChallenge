package com.example.korefugee.Guide

import android.content.Intent
import androidx.appcompat.app.AppCompatActivity
import android.os.Bundle
import com.example.korefugee.R
import com.example.korefugee.databinding.ActivityGuideDocument0202Binding
import com.example.korefugee.databinding.ActivityGuideDocument02Binding

class GuideDocument02_02Activity : AppCompatActivity() {
    // 뷰 바인딩을 위한 객체 획득
    lateinit var binding: ActivityGuideDocument0202Binding

    // 제목 intent로 받기

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        // 바인딩 기본 작업
        binding= ActivityGuideDocument0202Binding.inflate(layoutInflater)
        setContentView(binding.root)

        binding.back.setOnClickListener{
            val intent = Intent(this, GuideDocumenr01Activity::class.java)

            startActivity(intent)
        }
        binding.subcontent4.setOnClickListener {
            val intent = Intent(this, GuideDocument03Activity::class.java)
            intent.putExtra("title", "이의_상세")
            startActivity(intent)
        }

        binding.goButton1.setOnClickListener {
            val intent = Intent(this, GuideDocument03Activity::class.java)
            intent.putExtra("title", "이의")
            startActivity(intent)
            // 제목과 서류 제목 intent로 넘기기기
        }
    }
}